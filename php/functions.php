...
class ConfigService
{
	function getEstatisticasBairros(){
		
		require 'Conn.php';

		//	Captura apenas os bairros, distintos
		$sqlDistinctBairro = 'SELECT DISTINCT Bairro FROM clientes';
		$retornoSql = $ConnectDB->query( $sqlDistinctBairro);
		mysqli_close($ConnectDB); 	//	fecha conexão

		$DataBack = array();
		/* 
		// Define o status de acordo com o resultado da execução do SQL
		$Status = (mysqli_num_rows($Bairros) > 0) ? 000 : 001;
		if($Status > 000){
			$DataBack = array($Status);
			return $DataBack;
		} */
		foreach($retornoSql as $db){
			array_push($DataBack, $db['Bairro']);
		}
		
		return $DataBack;
	}

	function getTotalClientesPorBairro($bairro){
		
		require 'Conn.php';

		//	Captura apenas os bairros, distintos
		$sqlDistinctBairro = "SELECT * FROM clientes WHERE Bairro = '$bairro'";
		$retornoSql = $ConnectDB->query( $sqlDistinctBairro);
		mysqli_close($ConnectDB); 	//	fecha conexão
